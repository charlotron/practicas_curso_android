...
import com.worldpcs.tiendecitas4.util.FileUtils;
import com.worldpcs.tiendecitas4.util.JSONParser;
...
public class DataProvider {
	...
	/**
	 * URL para recuperar todas las tiendas
	 */
	protected static final String URL_GET_STORES=BASE_URL+"?query=stores";
	/**
	 * URL para recuperar las fotos
	 */
	protected static final String URL_GET_PHOTOS=BASE_URL+"?query=photos";	
	/**
	...
	/**
	 * Query para recuperar una tiendas
	 */
	public static final String QUERY_STORES="stores";
	/**
	 * Query para recuperar photos
	 */
	public static final String QUERY_PHOTOS="photos";	
	/**
	 * Query para recuperar photos
	 */
	public static final String QUERY_PHOTOS_REMOTE="photos_remote";		
	/**
	...
	/**
	 * Borrar comentario de tienda
	 */	
	public static final String ACTION_STORE_DELETE_COMMENT="del_store_comment";	
	/**
	...
	/**
	 * Borrar comentario de foto de tienda
	 */	
	public static final String ACTION_STORE_PHOTO_DELETE_COMMENT="del_store_photo_comment";		
	/**
	...
	public void updateAllData(Context context){
		updateAllData(context, true);
	}
	/**
	 * Descarga todos los datos
	 */
	public void updateAllData(Context context, boolean showDialog){
		new AsyncHandleData(context, QUERY_ALL, showDialog).execute();	
	}	
	/**
	 * Descarga todos los datos (Remoto o local)
	 */
	protected void internalUpdateAllData(){
		DBAdapter db = App.getInstance().getDb();
		//Cargamos los datos de la base de datos si están disponibles
		if(!db.isEmptyDb()){
			store_list=db.readStoreList();
			store_map.clear();
			for(Iterator<Store> it=store_list.iterator();it.hasNext();){
				Store store=it.next();
				store_map.put(store.getId(), store);
			}
			photo_list=db.readPhotoList();
			photo_map.clear();
			for(Iterator<Photo> it=photo_list.iterator();it.hasNext();){
				Photo photo=it.next();
	...
	/**
	 * Descarga todos los datos(Remoto)
	 */
	public void updateAllPhotos(Context context, boolean forceRemote){
		if(forceRemote)
			new AsyncHandleData(context, QUERY_PHOTOS_REMOTE).execute();
		else
			new AsyncHandleData(context, QUERY_PHOTOS).execute();	
	}	
	/**
	 * Descarga todas las fotos 
	 * @param b 
	 */
	protected void internalUpdateAllPhotos(boolean forceRemote){
		if(!forceRemote){
			DBAdapter db = App.getInstance().getDb();
		...
		// Recuperamos el array de datos
		JSONArray json_array = JSONParser.getJSONArrayFromURL(URL_GET_PHOTOS);
        internalUpdateData(json_array);
	}	
	/**
	...
	protected void internalAddStoreComment(Context context,int store_id, String comment){
		...
		//try {
			...
			//Creamos el comentario en local y lo actualizamos en la bd
			Comment com=new Comment();
			com.setComment(comment);
			App.getInstance().getDb().insertNewStoreComment(store_map.get(store_id), com);
	...
	protected void internalAddStorePhotoComment(Context context,int store_id,int photo_id, String comment){
		...
		//	JSONParser.getJSONObjectFromURL(url);	
			...
			Comment com=new Comment();
			com.setComment(comment);
			App.getInstance().getDb().insertNewPhotoComment(getCurrentStore().getPhotosMap().get(photo_id), com);
	 ...
	 * Borrar comentario de tienda
	 */
	public void delStoreComment(int store_id, int comment_id, DataUpdatedNotification listener) {
		new AsyncHandleData((Context) listener, ACTION_STORE_DELETE_COMMENT, store_id, comment_id).execute();			
	}	
	/**
	 * Borrar comentario de foto
	 */
	public void delStorePhotoComment(int store_id, int photo_id, int comment_id, DataUpdatedNotification listener) {
		new AsyncHandleData((Context) listener, ACTION_STORE_PHOTO_DELETE_COMMENT, store_id, photo_id, comment_id).execute();			
	}		
	/**
	 * 
	 */
	protected void internalDelStoreComment(Context context,int store_id, int comment_id){
		
		App.getInstance().getDb().delStoreComment(comment_id);
	 ...
	 * @param comment
	 */
	protected void internalDelStorePhotoComment(Context context,int store_id, int photo_id, int comment_id){
		//String url="";
		//try {
		//	url = String.format(URL_SEND_PHOTO_COMMENTS, photo_id, URLEncoder.encode(comment,"UTF-8"));
		//	JSONParser.getJSONObjectFromURL(url);	

		App.getInstance().getDb().delStorePhotoComment(comment_id);
	...
	protected void internalIncStoreFavCount(Context context,int store_id){
		//Incrementamos el valor de la tienda
		Store store=getCurrentStore();
		store.setFav_count(store.getFav_count()+1);
		
		App.getInstance().getDb().insertUpdateStore(store);
	...
	protected void internalIncStorePhotoFavCount(Context context,int store_id,int photo_id){
		//Incrementamos el valor de la tienda
		Store store=getCurrentStore();
		Photo photo = store.getPhotosList().get(0);
		photo.setFav_count(photo.getFav_count()+1);
		
		App.getInstance().getDb().insertUpdateStorePhotos(store);
	...
	public void internalAddCommunityPhoto(Context context, String picturePath) {
		if(FileUtils.SUCCESS_UPLOAD==FileUtils.uploadFile(picturePath, URL_SEND_PHOTO))
			new AsyncHandleData(context, QUERY_PHOTOS_REMOTE,false).execute();	
	}
	/**
	...
	protected void internalUpdateData(JSONArray json_array){
        ...
        //Vemos si tenemos que actualizar la base de datos
        App.getInstance().getDb().insertUpdateAllData();
	}
	/**
	 * Descarga toda la información nueva que pueda sobre una tienda concreta
	 */
	...
	protected class AsyncHandleData extends AsyncTask<Void,Void,Void>{
		...
		public AsyncHandleData(Context context, String type, int int1, int int2, int int3) {
			this.context=context;
			this.type=type;
			this.int1=int1;
			this.int2=int2;
		...
		public AsyncHandleData(Context context, String type, int int1, int int2, int int3, boolean showDialog) {
			this.context=context;
			this.type=type;
			this.int1=int1;
			this.int2=int2;
		...
		protected void onPreExecute() {
			super.onPreExecute();
			if(showDialog && context!=null && Activity.class.isInstance(context))
		...
		protected Void doInBackground(Void... params) {
			...
			else if(type==QUERY_STORES){
				//TODO
			}
			else if(type==QUERY_PHOTOS){
				internalUpdateAllPhotos(false);
			}
			else if(type==QUERY_PHOTOS_REMOTE){
				internalUpdateAllPhotos(true);
			}			
			...
			else if(type==ACTION_STORE_DELETE_COMMENT){
				internalDelStoreComment(context, int1, int2);
			}
			else if(type==ACTION_STORE_PHOTO_SEND_COMMENT){
				internalAddStorePhotoComment(context, int1,int2,str);				
			}
			else if(type==ACTION_STORE_PHOTO_DELETE_COMMENT){
				internalDelStorePhotoComment(context, int1, int2, int3);						
			}			
		...
		protected void onPostExecute(Void result) {
			...
			if(	type==QUERY_ALL || 
				type==QUERY_STORES || 
				type==QUERY_PHOTOS || 
				type==QUERY_PHOTOS_REMOTE || 
				type==QUERY_STORE || 
				type==QUERY_PHOTO || 
				type==QUERY_STORE_PHOTO){
				
				((DataUpdatedNotification)context).notifyActiveDataUpdated(type);
			}
			else if(type==ACTION_STORE_SEND_COMMENT || 
					type==ACTION_STORE_INC_FAV_COUNT || 
					type==ACTION_STORE_PHOTO_SEND_COMMENT || 
					type==ACTION_STORE_PHOTO_INC_FAV_COUNT || 
					type==ACTION_STORE_DELETE_COMMENT || 
					type==ACTION_STORE_PHOTO_DELETE_COMMENT){
	...
	public void setCurrentStore(int position){
		...
		this.current_store_id=store_list.get(position).getId();
	}
	/**
	 * Establece la tienda actual (en activo)
	 * @param store
	...
	public void setCurrentStore(Store store){
		if(current_store_id==store.getId()) return;
	...
	public Store getCurrentStore(){
		return store_map.get(current_store_id);
	}
